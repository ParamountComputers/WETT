...
namespace WETT.Controllers
{
    public class SaExciseDutyController : Controller
    {
        ...
        public JsonResult GetAll(JqGridViewModel request)
        {
            ...
            var SaExciseDutyData = from b in _context.InventoryTxDetails
                                      join a in _context.InventoryTxes on b.InventoryTxId equals a.InventoryTxId
                                      join c in _context.InventoryTxTypes on a.InventoryTxTypeId equals c.InventoryTxTypeId
                                      join d in _context.InventoryLocations on b.ToInventoryLocationId equals d.InventoryLocationId
                                      join e in _context.Products on b.ProductId equals e.ProductId
                ...
                foreach (Rule rule in request.searchfilters.rules.Where(a => !string.IsNullOrEmpty(a.data)))
                {
                    switch (rule.field)
                    {
                        case "date":
                            ...
                            break;
                        case "productName":
                            SaExciseDutyData = (IQueryable<SaExciseDutyViewModel>)SaExciseDutyData.Where(w => w.ProductName.Contains(rule.data));
                            break;
                        case "saCode":
                            SaExciseDutyData = (IQueryable<SaExciseDutyViewModel>)SaExciseDutyData.Where(w => w.SaCode.Contains(rule.data));
                            break;
                        case "comments":
                        ...
                        case "locationsDropdown":
                            ...
                            {
                                SaExciseDutyData = (IQueryable<SaExciseDutyViewModel>)SaExciseDutyData.Where(w => w.InventoryLocationId.ToString().Contains(rule.data));
                                break;
                            }
                        case "purchaseOrder":
                            SaExciseDutyData = (IQueryable<SaExciseDutyViewModel>)SaExciseDutyData.Where(w => w.PurchaseOrder.Contains(rule.data));
                            break;
